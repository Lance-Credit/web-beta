              ...
              <div class="rounded-lg border border-solid border-lance-black bg-[rgba(236,255,77,0.05)]">
                  ...
                  <ul class="pt-8 pb-6 px-4">
                      <li class="flex items-center justify-between pb-4 border-b border-solid border-b-lance-green-5">
                          ...
                          <p class="text-lance-black">{{ transactionTypeForUI }}</p>
                      </li>
                      ...
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          ...
                          <p class="text-lance-black">N{{ (notificationData?.data.balanceBefore)?.toLocaleString() }}</p>
                      </li>
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">
                              Balance after
                          </p>
                          <p class="text-lance-black">N{{ (notificationData?.data.balanceAfter)?.toLocaleString() }}</p>
                      </li>
                      <li class="flex items-center justify-between pt-4">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">
                              Transaction Date
                          </p>
                          <p class="text-lance-black">{{ new Date(notificationData?.data.createdAt).toLocaleDateString('en-GB', { day:"numeric", month:"short", year:"numeric" }) }}</p>
                      </li>
                  </ul>
              </div>
          </div>
...
<script setup lang="ts">
  ...
  const transactionTypeForUI = computed(() => {
    let txnTypeForUI;

    switch (props.notificationData?.data.subType) {
      case 'wallet_funding':
          txnTypeForUI = 'Wallet Top-Up'
          break;
      case 'wallet_withdrawal':
          txnTypeForUI = 'Wallet Withdrawal'
          break;
      case 'fees':
          txnTypeForUI = 'Fees'
          break;
      case 'loan_disbursement':
          txnTypeForUI = 'Loan Disbursement'
          break;
      case 'loan_repayment':
          txnTypeForUI = 'Loan Repayment'
          break;
      default:
          txnTypeForUI = 'Transaction'
          break;
    }
