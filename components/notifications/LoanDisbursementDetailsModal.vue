  ...
  <div @click.self="emit('@close-modal')" class="fixed z-20 top-0 left-0 w-screen h-screen backdrop-blur-[2px] flex items-center justify-center bg-lance-black-70">
      <div class="flex flex-col gap-6 w-full sm:w-[456px] h-full sm:h-auto mx-auto bg-white px-6 sm:px-10 p-10 sm:rounded-3xl overflow-y-scroll">
              ...
              <div class="rounded-lg border border-solid border-lance-black bg-[rgba(236,255,77,0.05)]">
                  ...
                  <ul class="pt-8 pb-6 px-4">
                      ...
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          ...
                          <p class="text-lance-black">{{ new Date(notificationData?.data.disbursedAt).toLocaleDateString('en-GB', { day:"numeric", month:"short", year:"numeric" }) }}</p>
                      </li>
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">Fee Amount</p>
                          <p class="text-lance-black">N{{ (notificationData?.data.feeAmount)?.toLocaleString() }}</p>
                      </li>
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">Monthly Repayment</p>
                          <p class="text-lance-black">N{{ (notificationData?.data.monthlyRepaymentAmount)?.toLocaleString() }}</p>
                      </li>
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">
                              Next Payment Date
                          </p>
                          <p class="text-lance-black">{{ new Date(notificationData?.data.nextPaymentDate).toLocaleDateString('en-GB', { day:"numeric", month:"short", year:"numeric" }) }}</p>
                      </li>
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">
                              Rate
                          ...
                          <p class="text-lance-black">
                            {{ notificationData?.data.rate }}% monthly
                        </p>
                      </li>
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">
                          ...
                          <p class="text-lance-black">
                            {{ notificationData?.data.tenure }} month(s)
                        </p>
                      </li>
                      <li class="flex items-center justify-between py-4 border-b border-solid border-b-lance-green-5">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">Total Interest Amount</p>
                          <p class="text-lance-black">N{{ (notificationData?.data.totalInterestAmount)?.toLocaleString() }}</p>
                      </li>
                      <li class="flex items-center justify-between pt-4">
                          <p class="text-lance-black-70 text-sm leading-[21px] flex items-center gap-2">
                              Total Repayment Amount
                          </p>
                          <p class="text-lance-black">N{{ (notificationData?.data.totalRepaymentAmount)?.toLocaleString() }}</p>
                      </li>
                  </ul>
              </div>
          </div>
          ...
          <div class="flex items-center gap-[14px]">
              <NuxtLink @click="emit('@close-modal')" to="/dashboard" class="btn btn-tertiary w-full">Back to Dashboard</NuxtLink>
